...
async def ocp_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ocp_find":
        ...
        conn = db.get_conn()
        cur = conn.cursor()
        cur.execute("SELECT user_id, username, balance, level, games_played, created_at FROM users ORDER BY created_at DESC LIMIT 20")
...
async def _handle_ocp_sessions(update: Update, session: dict, txt: str, uid: int) -> bool:
    ...
    if stype == "ocp_editgame_amount":
        ...
        conn = db.get_conn()
        c2 = conn.cursor()
        c2.execute("SELECT game_balance FROM users WHERE user_id=%s", (target_id,))
        row2 = c2.fetchone()
        if not row2:
            c2.close(); conn.close()
            await update.message.reply_text("❌ Пользователь не найден.")
            return True
        cur_val = row2[0] if row2[0] is not None else 0
        if txt.startswith("+") and txt[1:].isdigit():
            ...
            await update.message.reply_text("❌ Неверный формат. Используйте число, +100 или -100.")
            return True
        c2.execute("UPDATE users SET game_balance=%s WHERE user_id=%s", (new_val, target_id))
        conn.commit()
        c2.close(); conn.close()
        await update.message.reply_text(
