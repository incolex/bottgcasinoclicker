        ...
        if new_ban:
            ...
            try:
                await ctx.bot.send_message(target_id,
                    "🚫 <b>Ваш аккаунт заблокирован.</b>\n\nВы не можете использовать игру.",
                    parse_mode="HTML")
            except Exception: pass
        else:
...
async def _handle_ocp_sessions(update: Update, ctx: ContextTypes.DEFAULT_TYPE, session: dict, txt: str, uid: int) -> bool:
    ...
    if stype == "ocp_ban_id":
        ...
        await update.message.reply_text(f"🚫 Пользователь <code>{target_id}</code> (@{row['username']}) забанен и исключён из клана.", parse_mode="HTML")
        return True
    ...
    if stype == "ocp_unban_id":
        ...
        try:
            await ctx.bot.send_message(target_id,
                "✅ <b>Ваш аккаунт разблокирован!</b>\n\nВы снова можете пользоваться игрой. Нажмите /start.",
                parse_mode="HTML")
        except Exception: pass
        await update.message.reply_text(f"✅ Пользователь <code>{target_id}</code> (@{row['username']}) разбанен. Уведомление отправлено.", parse_mode="HTML")
        return True
