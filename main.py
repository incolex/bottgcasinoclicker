...
async def ocp_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ocp_find":
        ...
        cur = conn.cursor()
        # Игроки и баланс
        cur.execute("SELECT COUNT(*), COALESCE(SUM(game_balance),0), COALESCE(SUM(balance),0) FROM users WHERE is_banned=FALSE")
        row = cur.fetchone()
        total_players = row[0] or 0
        total_game_coins = int(row[1] or 0)
        total_bot_coins = int(row[2] or 0)
