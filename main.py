...
# Таймер конца сезона: unix timestamp когда сработает автосброс (0 = не задан)
...
# ── Автосброс сезона по таймеру ──────────────────────────────

async def _do_season_reset(bot):
    """Выполнить сброс сезона и оповестить всех игроков."""
    ...
    try:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS season_archive (
                id SERIAL PRIMARY KEY, season_num INTEGER DEFAULT 1,
                ended_at INTEGER, top_json TEXT
            )
        """)
        ...
        conn.rollback()
        logger.error(f"Auto season reset error: {e}")
        cur.close(); conn.close()
        return
    cur.close(); conn.close()
    ...
    for pid in all_ids:
        try:
            await bot.send_message(pid, announce, parse_mode="HTML")
            await asyncio.sleep(0.05)
        except Exception:
            pass
    logger.info(f"Season {next_season - 1} auto-reset done.")


async def _season_timer_coro(bot, end_ts: int):
    """Ждать до end_ts, затем сбросить сезон."""
...
def _ocp_main_kb():
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("👤 Найти / управлять игроком", callback_data="ocp_find")],
        [InlineKeyboardButton("💰 Изменить баланс (бот)",     callback_data="ocp_setbal")],
        [InlineKeyboardButton("🎮 Изменить баланс (игра)",    callback_data="ocp_setgame")],
        ...
        [InlineKeyboardButton("🌍 Глобальная статистика",     callback_data="ocp_globalstats")],
        [InlineKeyboardButton("👑 Выдать права OCP",          callback_data="ocp_addadmin"),
         InlineKeyboardButton("🔴 Забрать права",             callback_data="ocp_removeadmin")],
        [InlineKeyboardButton("👥 Администраторы",            callback_data="ocp_adminlist")],
        [InlineKeyboardButton(season_label,                   callback_data="ocp_season_timer")],
        [InlineKeyboardButton("🏁 Конец сезона (вручную)",   callback_data="ocp_season_confirm")],
    ])
...
async def ocp_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ocp_find":
        ...
        global _season_end_ts, _season_timer_task
        # Показать меню установки таймера сезона
        global _season_end_ts
        if _season_end_ts > 0:
            left = _season_end_ts - int(time.time())
            days = left // 86400
            hours = (left % 86400) // 3600
            mins = (left % 3600) // 60
            status = f"⏰ До конца сезона: <b>{days}д {hours}ч {mins}мин</b>"
        else:
            status = "⏰ Таймер не установлен."
        await q.edit_message_text(
            f"⏰ <b>Таймер конца сезона</b>\n\n{status}\n\n"
            "Выберите через сколько завершить сезон:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("✏️ Ввести вручную (часы)", callback_data="ocp_settimer_custom")],
                [InlineKeyboardButton("❌ Отменить таймер",        callback_data="ocp_canceltimer")],
        ...
        if val == "custom":
            ...
            await q.edit_message_text(
                "✏️ Введите количество часов до конца сезона (например: 48):",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_season_timer")]]))
            return
        days = int(val)
        _season_end_ts = int(time.time()) + days * 86400
        ...
        await q.edit_message_text(
            f"✅ Таймер установлен!\n\n🏁 Сезон завершится: <b>{dt}</b>\n"
            f"⏰ Через: <b>{days} дней</b>\n\nАвтосброс произойдёт автоматически.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]])
        )
...
async def _handle_ocp_sessions(update: Update, session: dict, txt: str, uid: int) -> bool:
    ...
    # OCP: установить таймер сезона вручную (часы)
