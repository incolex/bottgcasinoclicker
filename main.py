...
user_sessions: dict = {}      # {user_id: {type, ...}}
user_sessions_ts: dict = {}   # {user_id: timestamp} для TTL
spam_tracker: dict = {}
...
def _set_session(user_id: int, data: dict):
    ...
    user_sessions_ts[user_id] = time.time()
...
def _get_session(user_id: int):
    ...
    if not session:
        return None
    # Проверяем TTL
    ts = user_sessions_ts.get(user_id, 0)
    if time.time() - ts > SESSION_TTL:
        _del_session(user_id)
        user_sessions_ts.pop(user_id, None)
        return None
    return session


def _del_session(user_id: int):
...
def _load_admins_from_db():
    """Загрузить ID администраторов из БД при старте."""
    global _admin_ids
    try:
        conn = get_conn(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)")
        cur.execute("SELECT value FROM settings WHERE key='admin_ids'")
        row = cur.fetchone()
        if row and row["value"]:
            ids = {int(x) for x in row["value"].split(',') if x.strip().isdigit()}
            _admin_ids = ids | {DEVELOPER_ID}
        cur.close(); conn.close()
    except Exception as e:
        logger.warning(f"load_admins_from_db error: {e}")

def _save_admins_to_db():
    """Сохранить список администраторов в БД."""
    try:
        conn = get_conn(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)")
        cur.execute(
            "INSERT INTO settings(key,value) VALUES('admin_ids',%s) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value",
            (','.join(str(x) for x in _admin_ids),)
        )
        conn.commit(); cur.close(); conn.close()
    except Exception as e:
        logger.warning(f"save_admins_to_db error: {e}")

def _load_season_ts_from_db():
    """Загрузить таймер сезона из БД при старте и перезапустить корутину если нужно."""
    ...
    try:
        ...
        if row and row["value"]:
            ts = int(row["value"])
            if ts > time.time():  # таймер ещё не истёк
                _season_end_ts = ts
                # Корутину запустим позже через post_init чтобы event loop был готов
                logger.info(f"Season timer restored: ends at {datetime.datetime.fromtimestamp(ts).strftime('%d.%m.%Y %H:%M')}")
            else:
                _season_end_ts = 0  # истёк — сбрасываем
        cur.close(); conn.close()
    except Exception as e:
        logger.warning(f"load_season_ts error: {e}")
...
def check_spam(user_id):
    ...
    elif user_id in spam_tracker:
        del spam_tracker[user_id]
    return len(hist) > config.SPAM_LIMIT


def _cleanup_sessions():
    """Удалить устаревшие сессии из памяти (вызывается периодически)."""
    now = time.time()
    # spam_tracker: удалить пустые
    stale = [uid for uid, hist in spam_tracker.items()
             if not any(now - t < config.SPAM_WINDOW for t in hist)]
    for uid in stale:
...
async def profile_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "profile_withdraw_game":
        ...
        _set_session(user_id, {"type": "wgame_custom"})
        ...
        _set_session(user_id, {"type": "deposit_custom"})
...
async def coin_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if await ban_check(update):
        return
    user = update.effective_user
    ...
    row = db.get_user(user.id)
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("👑 Орёл", callback_data="coin_side_heads"),
         InlineKeyboardButton("🦅 Решка", callback_data="coin_side_tails")],
    ])
    await update.message.reply_text(
        f"🪙 <b>Монетка</b>\n💰 Баланс: <b>{row['balance']}</b>\n\n"
        f"Выберите сторону (выигрыш ×2):",
        reply_markup=kb, parse_mode="HTML"
    )


async def _coin_flip(update, user_id, side, bet):
    row = db.get_user(user_id)
    if row["balance"] < bet:
    ...
    result_side = games.flip_coin()
    won = result_side == side
    ...
    if won:
        ...
        result_text = f"😢 Не повезло. -{bet} монет"
    ...
    side_emoji = games.coin_result_emoji(side)
    result_emoji = games.coin_result_emoji(result_side)
    side_label = "Орёл 👑" if side == "heads" else "Решка 🦅"
    result_label = "Орёл 👑" if result_side == "heads" else "Решка 🦅"

    text = (
        f"🪙 <b>Монетка</b>  |  Ставка: {bet}\n\n"
        f"Ваш выбор: <b>{side_label}</b>\n"
        f"Выпало: <b>{result_label}</b>\n\n"
        f"{result_text}\n💰 Баланс: <b>{new_balance}</b>"
    )
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("👑 Орёл снова", callback_data=f"coin_play_heads_{bet}"),
         InlineKeyboardButton("🦅 Решка снова", callback_data=f"coin_play_tails_{bet}")],
        [InlineKeyboardButton("✏️ Другая ставка", callback_data="coin_newbet")],
    ])
    if update.callback_query:
...
async def coin_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("coin_side_"):
        # Выбрана сторона — показать выбор ставки
        side = data.split("_")[2]  # heads / tails
        _set_session(user_id, {"type": "coin_bet", "side": side})
        label = "Орёл 👑" if side == "heads" else "Решка 🦅"
        await q.edit_message_text(
            f"🪙 Вы выбрали: <b>{label}</b>\n\nВыберите ставку:",
            reply_markup=bet_keyboard(f"coin_bet_{side}_"), parse_mode="HTML"
        )

    elif data.startswith("coin_bet_"):
        # coin_bet_{side}_{amount}
        parts = data.split("_")
        side = parts[2]
        amount_str = parts[3]
        if amount_str == "custom":
            _set_session(user_id, {"type": "coin_custom_bet", "side": side})
            await q.edit_message_text("🪙 Введите вашу ставку числом:")
            return
        await _coin_flip(update, user_id, side, int(amount_str))

    elif data.startswith("coin_play_"):
        # coin_play_{side}_{bet} — быстрый повтор
        parts = data.split("_")
        side = parts[2]
        bet = int(parts[3])
        await _coin_flip(update, user_id, side, bet)

    elif data == "coin_newbet":
        row = db.get_user(user_id)
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("👑 Орёл", callback_data="coin_side_heads"),
             InlineKeyboardButton("🦅 Решка", callback_data="coin_side_tails")],
...
async def _bj_deal(update, user_id, bet):
    ...
    _set_session(user_id, {"type": "blackjack", "deck": deck, "player": player, "dealer": dealer, "bet": bet})
    db.update_balance(user_id, -bet)
...
async def bj_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action.startswith("bj_start_"):
        ...
        if part == "custom":
            _set_session(user_id, {"type": "bj_custom_bet"})
            await q.edit_message_text("🃏 Введите вашу ставку числом:")
            return
        bet = int(part)
...
async def roulette_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("rl_type_"):
        ...
        parts = data.split("_")
        # Формат: rl_bet_{bet_type}_{amount}  или  rl_bet_number_{num}_{amount}
        if parts[2] == "number" and len(parts) >= 5:
            bet_type   = "number"
            bet_val    = parts[3]       # выбранное число
            amount_str = parts[4]
        else:
            bet_type   = parts[2]
            bet_val    = None
            amount_str = parts[3]
        if amount_str == "custom":
            _set_session(user_id, {"type": "rl_custom_bet", "bet_type": bet_type, "bet_val": bet_val})
            await q.edit_message_text("🎯 Введите сумму ставки числом:")
            return
        await _roulette_spin(update, user_id, int(amount_str), bet_type, bet_val)
...
async def cmd_trade(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    to_id, to_username = row["user_id"], row["username"]
...
async def mines_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("mines_cnt_"):
        ...
        if existing:
            ...
            _set_session(user_id, {"type": "mines_pending_cnt", "new_cnt_data": data})
            return
        mines = int(data.split("_")[2])
        _set_session(user_id, {"type": "mines_bet", "mines": mines})
        await q.edit_message_text(
            f"💣 <b>Мины: {mines}</b>\n\nВыберите ставку:",
    ...
    if data.startswith("mines_bet_"):
        ...
        amount_str = parts[3]
        if amount_str == "custom":
            _set_session(user_id, {"type": "mines_custom_bet", "mines": mines})
            await q.edit_message_text("💣 Введите вашу ставку числом:")
            return
        if not amount_str.isdigit():
            return
...
async def _do_season_reset(bot):
    ...
    for i,r in enumerate(top3):
        ...
        try:
            ...
            logger.warning(f"Reward err: {e}")
    ns = 1  # default if DB fails
    ...
    try:
        ...
        _ns_row=cur.fetchone(); ns=_ns_row[0] if isinstance(_ns_row, tuple) else _ns_row["ns"]
        ...
        for u in all_users:
            ...
            reset_state = json.dumps({
                "coins": 0, "totalCoins": 0, "totalClicks": int(st.get("totalClicks", 0)),
                "clickPower": 1, "perSecond": 0,
                "diamonds": int(st.get("diamonds", 0)),
                "upgrades": [],
                "activeSkin": st.get("activeSkin", "default"),
                "activeBg": st.get("activeBg", "default"),
                "ownedSkins": st.get("ownedSkins", []),
                "ownedBgs": st.get("ownedBgs", []),
                "username": st.get("username", f"id{u['user_id']}"),
                "tgId": u["user_id"],
                "clan": None,
                "claimedAchs": st.get("claimedAchs", {}),
                "prestige": int(st.get("prestige", 0)),
                "prestigeMultiplier": float(st.get("prestigeMultiplier", 1.0)),
            }, ensure_ascii=False)
...
def _ocp_main_kb():
    season_label = "⏳ Установить таймер"
    if _season_end_ts > 0:
        left = _season_end_ts - int(time.time())
        if left > 0:
            d2=left//86400; h2=(left%86400)//3600
            season_label = f"⏰ Сезон: {d2}д {h2}ч"
        else:
            season_label = "⏰ Сезон истёк"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("👤 Найти игрока",           callback_data="ocp_find")],
        [InlineKeyboardButton("💰 Баланс (бот)",           callback_data="ocp_setbal"),
         InlineKeyboardButton("🎮 Баланс (игра)",          callback_data="ocp_setgame")],
        [InlineKeyboardButton("🚫 Забанить",               callback_data="ocp_ban"),
         InlineKeyboardButton("✅ Разбанить",              callback_data="ocp_unban")],
        [InlineKeyboardButton("🖱️ Снять бан кликера",     callback_data="ocp_unclick_ban")],
        [InlineKeyboardButton("📢 Рассылка текста",        callback_data="ocp_broadcast"),
         InlineKeyboardButton("🎁 Бонус всем",             callback_data="ocp_bonus_coins")],
        [InlineKeyboardButton("💎 Алмазы всем",            callback_data="ocp_bonus_diamonds")],
        [InlineKeyboardButton("📊 Топ-30",                 callback_data="ocp_top"),
         InlineKeyboardButton("🌍 Статистика",             callback_data="ocp_globalstats")],
        [InlineKeyboardButton("👑 Дать права",             callback_data="ocp_addadmin"),
         InlineKeyboardButton("🔴 Забрать права",          callback_data="ocp_removeadmin")],
        [InlineKeyboardButton("👥 Мои администраторы",     callback_data="ocp_adminlist")],
        [InlineKeyboardButton(season_label,                callback_data="ocp_season_timer"),
         InlineKeyboardButton("🏁 Конец сезона",           callback_data="ocp_season_confirm")],
    ])
...
def _apply_balance_change(target_id: int, val_str: str, game: bool = False):
    """Применить изменение баланса (+500/-200/1000). game=True → game_balance."""
    conn = db.get_conn(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    field = "game_balance" if game else "balance"
    cur.execute(f"SELECT {field}, game_state FROM users WHERE user_id=%s", (target_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close(); return
    cur_val = int(row[field] if row[field] is not None else 0)
    if val_str.startswith("+") and val_str[1:].isdigit():
        new_val = cur_val + int(val_str[1:])
    elif val_str.startswith("-") and val_str[1:].isdigit():
        new_val = max(0, cur_val - int(val_str[1:]))
    elif val_str.lstrip("-").isdigit():
        new_val = max(0, int(val_str))
    else:
        cur.close(); conn.close(); return
    # При изменении game_balance — обновляем и game_state.coins чтобы клиент сразу получил правильный баланс
    if game and row.get("game_state"):
        try:
            st = json.loads(row["game_state"])
            st["coins"] = new_val
            cur.execute(
                "UPDATE users SET game_balance=%s, game_state=%s, updated_at=%s WHERE user_id=%s",
                (new_val, json.dumps(st, ensure_ascii=False), int(time.time()), target_id)
            )
        except Exception:
            cur.execute(f"UPDATE users SET {field}=%s, updated_at=%s WHERE user_id=%s",
                        (new_val, int(time.time()), target_id))
    else:
        cur.execute(f"UPDATE users SET {field}=%s, updated_at=%s WHERE user_id=%s",
                    (new_val, int(time.time()), target_id))
    conn.commit(); cur.close(); conn.close()
...
async def ocp_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ocp_find":
        ...
        for r in recent:
            ...
            gb = r["game_balance"] or 0
            btns.append([InlineKeyboardButton(
                f"{ban_mark}@{r['username']} · {gb:,}🎮",
                callback_data=f"ocp_info_{r['user_id']}"
            )])
        btns.append([InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")])
        await q.edit_message_text(
            "🔍 <b>Найти игрока</b>\n\n"
            "👆 Быстрый доступ (недавние) — нажмите кнопку\n"
            "⌨️ Или <b>напишите</b> @username или числовой ID:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(btns))

    elif data == "ocp_setbal":
        _set_session(uid, {"type": "ocp_setbal_id"})
        await q.edit_message_text(
            "💰 <b>Изменить баланс бота</b>\n\nВведите <b>числовой ID</b> пользователя:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]]))

    elif data == "ocp_setgame":
        _set_session(uid, {"type": "ocp_setgame_id"})
        await q.edit_message_text(
            "🎮 <b>Изменить баланс игры</b>\n\nВведите <b>числовой ID</b> пользователя:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]]))

    elif data == "ocp_ban":
        _set_session(uid, {"type": "ocp_ban_id"})
        await q.edit_message_text(
            "🚫 <b>Забанить игрока</b>\n\nВведите <b>числовой ID</b> пользователя:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]]))
        ...
        _set_session(uid, {"type": "ocp_unban_id"})
        await q.edit_message_text(
            "✅ <b>Разбанить игрока</b>\n\nВведите <b>числовой ID</b> пользователя:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]]))

    elif data == "ocp_unclick_ban":
        _set_session(uid, {"type": "ocp_unclick_ban_id"})
        await q.edit_message_text(
            "🖱️ <b>Снять бан кликера</b>\n\nВведите <b>числовой ID</b> пользователя:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]]))


    elif data == "ocp_broadcast":
        _set_session(uid, {"type": "ocp_broadcast"})
        await q.edit_message_text(
            "📢 <b>Рассылка текста</b>\n\nВведите сообщение (HTML поддерживается).\n\n"
            "Пример: <code>&lt;b&gt;Новости!&lt;/b&gt; Текст...</code>\n\n"
            "✏️ Напишите текст в ответном сообщении:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]]))

    elif data == "ocp_top" or data.startswith("ocp_top_page_"):
        page = 0
        if data.startswith("ocp_top_page_"):
            try: page = int(data.split("ocp_top_page_")[1])
            except: page = 0
        rows = db.get_top_users(100)
        per_page = 10
        total = len(rows)
        ...
        for i, row in enumerate(page_rows, start + 1):
            ...
            ban_mark = "🚫 " if row.get("is_banned") else ""
            label = f"{medal} {ban_mark}@{row['username']} · {game_bal:,}🎮"
            btns.append([InlineKeyboardButton(label, callback_data=f"ocp_info_{row['user_id']}")])
        # Навигация
        nav = []
        if page > 0:
            nav.append(InlineKeyboardButton("◀️ Назад", callback_data=f"ocp_top_page_{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="ocp_noop"))
        if page < total_pages - 1:
            nav.append(InlineKeyboardButton("Вперёд ▶️", callback_data=f"ocp_top_page_{page+1}"))
        if nav:
            btns.append(nav)
        btns.append([InlineKeyboardButton("◀️ Главное меню", callback_data="ocp_back")])
        await q.edit_message_text(
            f"📊 <b>Топ игроков</b> (стр. {page+1}/{total_pages}) · {total} игроков\n\nНажмите для управления:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(btns))
        ...
        rows = cur.fetchall()
        cur.close(); conn.close()
        lines = ["📋 <b>Топ-20 игроков</b> (нажмите для управления):\n"]
        btns = []
        for row in rows:
            ban_mark = "🚫 " if row["is_banned"] else ""
            gb = row["game_balance"] or 0
            label = f"{ban_mark}@{row['username']} · {gb:,}🎮"
            btns.append([InlineKeyboardButton(label, callback_data=f"ocp_info_{row['user_id']}")])
        btns.append([InlineKeyboardButton("🔍 Найти по нику/ID", callback_data="ocp_find")])
        btns.append([InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")])
        await q.edit_message_text(
            "📋 <b>Игроки</b> (по игровому балансу)\n\nНажмите на игрока для управления:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(btns))
        ...
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        # Игроки и баланс
        cur.execute("SELECT COUNT(*) AS cnt, COALESCE(SUM(game_balance),0) AS sg, COALESCE(SUM(balance),0) AS sb FROM users WHERE is_banned=FALSE")
        row = cur.fetchone()
        total_players = row["cnt"] or 0
        total_game_coins = int(row["sg"] or 0)
        total_bot_coins = int(row["sb"] or 0)
        ...
        text = (
            ...
            f"👥 Игроков: <b>{_gfmt(total_players)}</b>\n"
            f"⚔️ Кланов: <b>{clan_count}</b>\n\n"
            f"🖱 Всего кликов: <b>{_gfmt(total_clicks)}</b>\n"
            f"💰 Заработано монет (всего): <b>{_gfmt(total_earned)}</b>\n"
            f"🎮 В игре сейчас: <b>{_gfmt(total_game_coins)}</b>\n"
            f"🤖 В боте сейчас: <b>{_gfmt(total_bot_coins)}</b>\n"
            f"⚡ Монет/сек (вместе): <b>{_gfmt(total_per_sec)}</b>\n\n"
            f"🔄 Трейдов: <b>{trade_count}</b>  |  Объём: <b>{_gfmt(trade_volume)} G</b>\n"
        ...
        if q.from_user.id != DEVELOPER_ID:
            ...
            return
        _set_session(uid, {"type": "ocp_addadmin"})
        await q.edit_message_text(
        ...
        if q.from_user.id != DEVELOPER_ID:
            ...
            return
        _set_session(uid, {"type": "ocp_removeadmin"})
        await q.edit_message_text(
        ...
        _season_timer_task=asyncio.create_task(_season_timer_coro(ctx.bot, _season_end_ts))
        try:
            import httpx as _hx
    ...
    elif data == "ocp_bonus_coins":
        _set_session(uid, {"type": "ocp_bonus_coins"})
        await q.edit_message_text(
            "💰 <b>Бонус монетами всем игрокам</b>\n\n"
            "Введите сумму монет. Каждый получит на бот-баланс.\n"
            "Пример: <code>5000</code>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]]))

    elif data == "ocp_bonus_diamonds":
        _set_session(uid, {"type": "ocp_bonus_diamonds"})
        await q.edit_message_text(
            "💎 <b>Бонус алмазами всем игрокам</b>\n\n"
            "Введите количество алмазов. Каждый получит в игру.\n"
            "Пример: <code>10</code>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="ocp_back")]]))
        ...
        try:
            ...
            """)
            cur.execute("SELECT COALESCE(MAX(season_num),0)+1 AS ns FROM season_archive")
            _ns_row2=cur.fetchone(); next_season=_ns_row2[0] if isinstance(_ns_row2, tuple) else _ns_row2["ns"]
            top_json = json.dumps([
        ...
        # Сбрасываем таймер сезона
        _season_end_ts = 0
        if _season_timer_task and not _season_timer_task.done():
            _season_timer_task.cancel()
        _season_timer_task = None
        # Сбрасываем таймер и в БД
        try:
            conn2 = db.get_conn(); cur2 = conn2.cursor()
            cur2.execute("INSERT INTO settings(key,value) VALUES('season_end_ts','0') ON CONFLICT(key) DO UPDATE SET value='0'")
            conn2.commit(); cur2.close(); conn2.close()
        except Exception as e:
            logger.warning(f"season_end_ts reset error: {e}")
        ...
        upd = datetime.datetime.fromtimestamp(row["updated_at"]).strftime("%d.%m.%Y %H:%M") if row.get("updated_at") else "?"
        game_bal = row["game_balance"] if row["game_balance"] is not None else 0
        ban_status = "🚫 Забанен" if row.get("is_banned") else "✅ Активен"
        text = (
            f"👤 <b>@{row['username']}</b>\n"
            f"🪪 ID: <code>{row['user_id']}</code>  |  {ban_status}\n\n"
            f"💰 Бот: <b>{row['balance']:,}</b>  🎮 Игра: <b>{game_bal:,}</b>\n"
            f"⭐ Ур.{row['level']}  XP: {row['xp']}  |  🎮 {row['games_played']} игр ({row['wins']}✅/{row['losses']}❌)\n\n"
            f"📅 Регистрация: {reg}\n"
            f"🕐 Последняя активность: {upd}"
        )
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💰 Изм. баланс бота", callback_data=f"ocp_editbal_{target_id}"),
             InlineKeyboardButton("🎮 Изм. баланс игры", callback_data=f"ocp_editgame_{target_id}")],
            [ban_btn,
             InlineKeyboardButton("📋 К списку", callback_data="ocp_list")],
            [InlineKeyboardButton("◀️ Главное меню", callback_data="ocp_back")],
        ])
        ...
        if not row:
            await q.answer("Пользователь не найден.", show_alert=True)
            return
        # Забанить администратора может только главный разработчик
        if target_id in _admin_ids and q.from_user.id != DEVELOPER_ID:
            await q.answer("⛔ Забанить администратора может только главный разработчик.", show_alert=True)
            return
        # Главного разработчика не может забанить никто
        if target_id == DEVELOPER_ID:
            await q.answer("⛔ Нельзя забанить главного разработчика.", show_alert=True)
            return
        new_ban = not bool(row.get("is_banned"))
        db.set_ban(target_id, new_ban)
        if new_ban:
            # Кикаем из клана при бане
            try:
                conn = db.get_conn(); cur = conn.cursor()
                cur.execute("DELETE FROM clan_members WHERE user_id=%s", (target_id,))
                cur.execute("UPDATE users SET clan_id=NULL WHERE user_id=%s", (target_id,))
                conn.commit(); cur.close(); conn.close()
            except Exception: pass
        action_text = "🚫 Забанен" if new_ban else "✅ Разбанен"
        await q.answer(f"{action_text}: @{row['username']}", show_alert=True)
        # Уведомить игрока ОДИН РАЗ — только если статус действительно изменился
        try:
            msg = "🚫 <b>Ваш аккаунт заблокирован администратором.</b>" if new_ban else "✅ <b>Ваш аккаунт разблокирован!</b> Нажмите /start."
            await ctx.bot.send_message(target_id, msg, parse_mode="HTML")
        except Exception: pass
        # Обновить карточку напрямую без рекурсии
        row2 = db.get_user(target_id)
        if not row2:
            return
        reg = datetime.datetime.fromtimestamp(row2["created_at"]).strftime("%d.%m.%Y %H:%M") if row2["created_at"] else "?"
        upd = datetime.datetime.fromtimestamp(row2["updated_at"]).strftime("%d.%m.%Y %H:%M") if row2.get("updated_at") else "?"
        game_bal2 = row2["game_balance"] if row2["game_balance"] is not None else 0
        ban_status2 = "🚫 Забанен" if row2.get("is_banned") else "✅ Активен"
        text2 = (
            f"👤 <b>@{row2['username']}</b>\n"
            f"🪪 ID: <code>{row2['user_id']}</code>  |  {ban_status2}\n\n"
            f"💰 Бот: <b>{row2['balance']:,}</b>  🎮 Игра: <b>{game_bal2:,}</b>\n"
            f"⭐ Ур.{row2['level']}  XP: {row2['xp']}  |  🎮 {row2['games_played']} игр ({row2['wins']}✅/{row2['losses']}❌)\n\n"
            f"📅 Регистрация: {reg}\n"
            f"🕐 Последняя активность: {upd}"
        )
        ban_btn2 = InlineKeyboardButton(
            "✅ Разбанить" if row2.get("is_banned") else "🚫 Забанить",
            callback_data=f"ocp_toggleban_{target_id}"
        )
        kb2 = InlineKeyboardMarkup([
            [InlineKeyboardButton("💰 Изм. баланс бота", callback_data=f"ocp_editbal_{target_id}"),
             InlineKeyboardButton("🎮 Изм. баланс игры", callback_data=f"ocp_editgame_{target_id}")],
            [ban_btn2,
             InlineKeyboardButton("◀️ К топу", callback_data="ocp_top")],
            [InlineKeyboardButton("◀️ Главное меню", callback_data="ocp_back")],
        ])
        await q.edit_message_text(text2, reply_markup=kb2, parse_mode="HTML")

    elif data.startswith("ocp_editbal_"):
        target_id = int(data.split("_")[2])
        _set_session(uid, {"type": "ocp_editbal_amount", "target_id": target_id})
        row = db.get_user(target_id)
        cur_bal = row["balance"] if row else 0
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("+1K",    callback_data=f"ocp_balset_{target_id}_+1000"),
             InlineKeyboardButton("+5K",    callback_data=f"ocp_balset_{target_id}_+5000"),
             InlineKeyboardButton("+10K",   callback_data=f"ocp_balset_{target_id}_+10000")],
            [InlineKeyboardButton("+50K",   callback_data=f"ocp_balset_{target_id}_+50000"),
             InlineKeyboardButton("+100K",  callback_data=f"ocp_balset_{target_id}_+100000"),
             InlineKeyboardButton("Обнулить", callback_data=f"ocp_balset_{target_id}_0")],
            [InlineKeyboardButton("◀️ Назад", callback_data=f"ocp_info_{target_id}")],
        ])
        await q.edit_message_text(
            f"💰 <b>Баланс бота @{row['username'] if row else target_id}</b>\n\n"
            f"Текущий: <b>{cur_bal:,}</b>\n\n"
            f"Быстрые кнопки ↓ или напишите: +500 / -200 / 1000",
            parse_mode="HTML", reply_markup=kb)

    elif data.startswith("ocp_editgame_"):
        target_id = int(data.split("_")[2])
        _set_session(uid, {"type": "ocp_editgame_amount", "target_id": target_id})
        row = db.get_user(target_id)
        cur_bal = row["game_balance"] if row and row["game_balance"] is not None else 0
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("+1K",    callback_data=f"ocp_gameset_{target_id}_+1000"),
             InlineKeyboardButton("+10K",   callback_data=f"ocp_gameset_{target_id}_+10000"),
             InlineKeyboardButton("+100K",  callback_data=f"ocp_gameset_{target_id}_+100000")],
            [InlineKeyboardButton("+1M",    callback_data=f"ocp_gameset_{target_id}_+1000000"),
             InlineKeyboardButton("+10M",   callback_data=f"ocp_gameset_{target_id}_+10000000"),
             InlineKeyboardButton("Обнулить", callback_data=f"ocp_gameset_{target_id}_0")],
            [InlineKeyboardButton("◀️ Назад", callback_data=f"ocp_info_{target_id}")],
        ])
        await q.edit_message_text(
            f"🎮 <b>Игровой баланс @{row['username'] if row else target_id}</b>\n\n"
            f"Текущий: <b>{cur_bal:,}</b>\n\n"
            f"Быстрые кнопки ↓ или напишите: +500 / -200 / 1000",
            parse_mode="HTML", reply_markup=kb)

    elif data.startswith("ocp_balset_"):
        # ocp_balset_{target_id}_{value}
        parts = data.split("_")
        target_id = int(parts[2]); val_str = parts[3]
        _apply_balance_change(target_id, val_str, game=False)
        row = db.get_user(target_id)
        await q.answer(f"✅ Баланс обновлён: {row['balance']:,}", show_alert=True)
        # Показываем карточку игрока инлайн
        if row:
            reg = datetime.datetime.fromtimestamp(row["created_at"]).strftime("%d.%m.%Y %H:%M") if row["created_at"] else "?"
            upd = datetime.datetime.fromtimestamp(row["updated_at"]).strftime("%d.%m.%Y %H:%M") if row.get("updated_at") else "?"
            game_bal = row["game_balance"] if row["game_balance"] is not None else 0
            ...
            text2 = (
                ...
                f"🪪 ID: <code>{row['user_id']}</code>  |  {ban_status}\n\n"
                f"💰 Бот: <b>{row['balance']:,}</b>  🎮 Игра: <b>{game_bal:,}</b>\n"
                f"⭐ Ур.{row['level']}  XP: {row['xp']}\n\n"
                f"📅 Регистрация: {reg}\n"
                f"🕐 Последняя активность: {upd}\n\n"
                f"✅ <b>Баланс бота обновлён!</b>"
            )
            ban_btn2 = InlineKeyboardButton(
                "✅ Разбанить" if row.get("is_banned") else "🚫 Забанить",
                callback_data=f"ocp_toggleban_{target_id}"
            ...
            kb2 = InlineKeyboardMarkup([
                [InlineKeyboardButton("💰 Изм. баланс бота", callback_data=f"ocp_editbal_{target_id}"),
                 InlineKeyboardButton("🎮 Изм. баланс игры", callback_data=f"ocp_editgame_{target_id}")],
                [ban_btn2, InlineKeyboardButton("◀️ К топу", callback_data="ocp_top")],
                [InlineKeyboardButton("◀️ Главное меню", callback_data="ocp_back")],
            ])
            await q.edit_message_text(text2, reply_markup=kb2, parse_mode="HTML")

    elif data.startswith("ocp_gameset_"):
        parts = data.split("_")
        target_id = int(parts[2]); val_str = parts[3]
        _apply_balance_change(target_id, val_str, game=True)
        row = db.get_user(target_id)
        gb = row["game_balance"] or 0
        await q.answer(f"✅ Игровой баланс: {gb:,}", show_alert=True)
        # Показываем карточку игрока инлайн
        if row:
            reg = datetime.datetime.fromtimestamp(row["created_at"]).strftime("%d.%m.%Y %H:%M") if row["created_at"] else "?"
            upd = datetime.datetime.fromtimestamp(row["updated_at"]).strftime("%d.%m.%Y %H:%M") if row.get("updated_at") else "?"
...
async def _handle_ocp_sessions(update: Update, ctx: ContextTypes.DEFAULT_TYPE, session: dict, txt: str, uid: int) -> bool:
    global _season_end_ts, _season_timer_task
    ...
    if stype == "ocp_ban_id":
        ...
        target_id = int(txt)
        if target_id == DEVELOPER_ID:
            await update.message.reply_text("⛔ Нельзя забанить главного разработчика.")
            return True
        if target_id in _admin_ids and uid != DEVELOPER_ID:
            await update.message.reply_text("⛔ Забанить администратора может только главный разработчик.")
            return True
        row = db.get_user(target_id)
        if not row:
            await update.message.reply_text("❌ Пользователь не найден.")
        ...
        async def _notify(uid): 
            try: await ctx.bot.send_message(uid, "🚫 <b>Ваш аккаунт заблокирован.</b>", parse_mode="HTML")
            except Exception: pass
        asyncio.create_task(_notify(target_id))
        return True
    ...
    if stype == "ocp_unban_id":
        ...
        async def _notify_unban(uid):
            try: await ctx.bot.send_message(uid, "✅ <b>Ваш аккаунт разблокирован!</b> Нажмите /start.", parse_mode="HTML")
            except Exception: pass
        asyncio.create_task(_notify_unban(target_id))
        return True

    # OCP: снять бан кликера
    if stype == "ocp_unclick_ban_id":
        _del_session(uid)
        if not txt.isdigit():
            await update.message.reply_text("❌ Введите числовой ID.")
            return True
        ...
        try:
            await ctx.bot.send_message(
                target_id,
                "✅ <b>Бан кликера снят администратором!</b>\n\nВы снова можете кликать в игре.",
                parse_mode="HTML"
            )
        except Exception: pass
        await update.message.reply_text(
            f"✅ Бан кликера снят у <code>{target_id}</code> (@{row['username']}).",
            parse_mode="HTML"
        )
        return True
    ...
    if stype == "ocp_setbal_id":
        if not txt.isdigit():
            await update.message.reply_text("❌ Введите числовой ID.")
            return True
        _set_session(uid, {"type": "ocp_editbal_amount", "target_id": int(txt)})
        await update.message.reply_text(
    ...
    if stype == "ocp_setgame_id":
        if not txt.isdigit():
            await update.message.reply_text("❌ Введите числовой ID.")
            return True
        _set_session(uid, {"type": "ocp_editgame_amount", "target_id": int(txt)})
        await update.message.reply_text(
    ...
    if stype == "ocp_editbal_amount":
        ...
        _apply_balance_change(target_id, txt.strip(), game=False)
        row = db.get_user(target_id)
        if not row:
            await update.message.reply_text("❌ Пользователь не найден.")
            return True
        await update.message.reply_text(
            f"✅ Баланс @{row['username']} → <b>{row['balance']:,} монет</b>.", parse_mode="HTML"
        )
        return True
    ...
    if stype == "ocp_editgame_amount":
        _del_session(uid)
        target_id = session["target_id"]
        _apply_balance_change(target_id, txt.strip(), game=True)
        row = db.get_user(target_id)
        if not row:
            await update.message.reply_text("❌ Пользователь не найден.")
            return True
        gb = row["game_balance"] or 0
        await update.message.reply_text(
            f"✅ Игровой баланс @{row['username']} → <b>{gb:,} монет</b>.", parse_mode="HTML"
        )
        return True
    ...
    if stype == "ocp_bonus_coins":
        _del_session(uid)
        if not txt.strip().isdigit() or int(txt.strip()) <= 0:
            await update.message.reply_text("❌ Введите положительное число монет.")
            return True
        amount = int(txt.strip())
        conn = db.get_conn(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT COUNT(*) AS cnt FROM users WHERE is_banned=FALSE")
        cnt_row = cur.fetchone()
        cnt = int(cnt_row["cnt"] if cnt_row and cnt_row.get("cnt") is not None else 0)
        cur.execute("UPDATE users SET game_balance=game_balance+%s WHERE is_banned=FALSE", (amount,))
        conn.commit(); cur.close(); conn.close()
        await update.message.reply_text(
            f"✅ Выдано <b>{amount:,} монет</b> каждому из <b>{cnt}</b> игроков.", parse_mode="HTML")
        sent = 0
        for pid in db.get_all_user_ids():
            try:
                await ctx.bot.send_message(pid,
                    f"🎁 <b>Подарок от администрации!</b>\n+<b>{amount:,} монет</b> зачислено на ваш <b>игровой баланс</b>! (открой игру чтобы увидеть)",
                    parse_mode="HTML")
                sent += 1
                await asyncio.sleep(0.05)
            except Exception: pass
    ...
    if stype == "ocp_bonus_diamonds":
        _del_session(uid)
        if not txt.strip().isdigit() or int(txt.strip()) <= 0:
            await update.message.reply_text("❌ Введите положительное число алмазов.")
            return True
        amount = int(txt.strip())
        conn = db.get_conn(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        for r in raw:
            try:
                st = json.loads(r["game_state"]) if r.get("game_state") else {}
            except Exception:
                st = {}
            st["diamonds"] = int(st.get("diamonds") or 0) + amount
            updates.append((json.dumps(st, ensure_ascii=False), r["user_id"]))
            upd += 1
        for gs, uid2 in updates:
            cur.execute("UPDATE users SET game_state=%s WHERE user_id=%s", (gs, uid2))
        conn.commit(); cur.close(); conn.close()
        await update.message.reply_text(
            f"✅ Выдано <b>{amount} 💎</b> каждому из <b>{upd}</b> игроков.", parse_mode="HTML")
        sent = 0
        for pid in db.get_all_user_ids():
            try:
                await ctx.bot.send_message(pid,
                    f"🎁 <b>Подарок от администрации!</b>\n+<b>{amount} 💎 алмазов</b> в игру!",
                    parse_mode="HTML")
                sent += 1
                await asyncio.sleep(0.05)
            except Exception: pass
...
async def text_router(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if session:
        if await _handle_ocp_sessions(update, ctx, session, txt, user.id):
            return
        stype = session.get("type")

        if stype == "coin_custom_bet":
            ...
            return await _coin_flip(update, user.id, side, bet)
...
async def _post_init(app):
    """Запускается после старта event loop — можно создавать корутины."""
    global _season_timer_task
    if _season_end_ts > 0 and _season_end_ts > time.time():
        _season_timer_task = asyncio.create_task(_season_timer_coro(app.bot, _season_end_ts))
