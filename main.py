...
async def coin_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("coin_side_"):
        ...
        user_sessions[user_id] = {"type": "coin_bet", "side": side}
        ...
        if amount_str == "custom":
            user_sessions[user_id] = {"type": "coin_custom_bet", "side": side}
...
async def roulette_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("rl_type_"):
        ...
        if parts[2] == "number" and len(parts) >= 5:
            ...
            amount_str = parts[3]
        if amount_str == "custom":
            user_sessions[user_id] = {"type": "rl_custom_bet", "bet_type": bet_type, "bet_val": bet_val}
...
async def ocp_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ocp_find":
        ...
        user_sessions[uid] = {"type": "ocp_unclick_ban_id"}
        await q.edit_message_text(
        ...
        user_sessions[uid] = {"type": "ocp_bonus_coins"}
        await q.edit_message_text(
        ...
        user_sessions[uid] = {"type": "ocp_bonus_diamonds"}
        await q.edit_message_text(
...
async def _post_init(app):
    ...
    if _season_end_ts > 0 and _season_end_ts > time.time():
        _season_timer_task = asyncio.ensure_future(_season_timer_coro(app.bot, _season_end_ts))
