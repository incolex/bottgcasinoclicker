...
def _ocp_main_kb():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📋 Игроки",                 callback_data="ocp_list")],
...
async def ocp_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ocp_find":
        ...
        # Уведомить игрока в фоне — не ждём
        async def _notify_ban(uid, banned):
            try:
                msg = "🚫 <b>Ваш аккаунт заблокирован.</b>" if banned else "✅ <b>Ваш аккаунт разблокирован!</b> Нажмите /start."
                await ctx.bot.send_message(uid, msg, parse_mode="HTML")
            except Exception: pass
        asyncio.create_task(_notify_ban(target_id, new_ban))
        # Обновить карточку — перечитываем обновлённый data callback
