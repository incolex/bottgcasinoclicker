...
async def _handle_ocp_sessions(update: Update, session: dict, txt: str, uid: int) -> bool:
    ...
    if stype=="ocp_bonus_all_amount":
        ...
        if is_diamonds:
            ...
            raw_rows=cur.fetchall(); upd=0
            rows=[dict(zip([d[0] for d in cur.description],r)) for r in raw_rows]
            for r in rows:
            ...
            cur.execute("SELECT COUNT(*) FROM users WHERE is_banned=FALSE"); cnt=cur.fetchone()[0]
