...
def make_main_kb(game_balance: int = 0):
    """Главная клавиатура с кнопкой WebApp. Передаёт game_balance через URL."""
    url = f"{WEBAPP_URL}?startapp=gb_{game_balance}"
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text, reply_markup=make_main_kb(game_bal), parse_mode="HTML")
