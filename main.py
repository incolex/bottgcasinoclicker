...
async def show_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"💰 Баланс: <b>{row['balance']} монет</b>\n"
...
def _mines_render(revealed: list, mines: list, show_all: bool = False) -> str:
    """Строит текстовое поле 5×5."""
    lines = []
    for r in range(5):
        row_str = ""
        for c in range(5):
            ...
            if idx in revealed:
                row_str += "💎"
            elif show_all and idx in mines:
                row_str += "💣"
            elif show_all:
                row_str += "⬜"
            else:
...
def _mines_kb(revealed: list, mines: list, game_id: str) -> InlineKeyboardMarkup:
    """Кнопки поля — только неоткрытые клетки."""
    rows = []
    for r in range(5):
        row = []
        for c in range(5):
        ...
        rows.append(row)
    rows.append([InlineKeyboardButton("💰 Забрать выигрыш", callback_data=f"mines_cashout_{game_id}")])
    return InlineKeyboardMarkup(rows)


def _mines_multiplier(revealed_count: int) -> float:
    """Множитель в зависимости от числа открытых ячеек (с учётом 5 мин)."""
    safe_total = 25 - MINES_COUNT  # 20
    mult = 1.0
    for i in range(revealed_count):
        mult *= (safe_total - i) / (25 - i)
    # Инвертируем — чем больше открыто, тем выше выигрыш
    if mult <= 0:
        return 1.0
    payout = round(1.0 / mult * 0.97, 2)  # 3% комиссия казино
    return max(payout, 1.0)
...
async def mines_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"На поле 25 клеток, среди которых {MINES_COUNT} мин.\n"
        f"Открывайте клетки — множитель растёт!\n\n"
        f"Выберите ставку:",
        reply_markup=bet_keyboard("mines_bet_"), parse_mode="HTML"
    )
...
async def mines_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Нажата уже открытая клетка — игнорируем
    if data == "mines_noop":
        return
    ...
    if data.startswith("mines_bet_"):
        part = data.split("_")[2]
        if part == "custom":
            user_sessions[user_id] = {"type": "mines_custom_bet"}
            await q.edit_message_text("💣 Введите вашу ставку числом:")
            return
        bet = int(part)
        await _mines_new_game(q, user_id, bet)
        return
    ...
    if data.startswith("mines_open_"):
        ...
        idx = int(parts[3])
        session = user_sessions.get(user_id)
        if not session or session.get("type") != "mines" or session.get("game_id") != game_id:
            await q.edit_message_text("❌ Сессия не найдена. Начните заново.")
            return
        mines = session["mines"]
        ...
        if idx in mines:
            # Взрыв!
            revealed_copy = list(revealed)
            db.record_game(user_id, False)
            del user_sessions[user_id]
            field = _mines_render(revealed_copy, mines, show_all=True)
            await q.edit_message_text(
                f"💥 <b>БУМ! Вы наступили на мину!</b>\n\n{field}\n\n"
                f"❌ Проиграно: <b>{bet} монет</b>\n"
                f"💰 Баланс: <b>{db.get_user(user_id)['balance']} монет</b>",
                parse_mode="HTML"
            )
            return
        ...
        field = _mines_render(revealed, mines)
        kb = _mines_kb(revealed, mines, game_id)
        await q.edit_message_text(
            f"💣 <b>Мины 5×5</b>  |  Ставка: {bet}\n\n"
            f"{field}\n\n"
            f"✅ Открыто: {len(revealed)} / 20\n"
            f"📈 Множитель: ×{mult}\n"
    ...
    if data.startswith("mines_cashout_"):
        ...
        bet = session["bet"]
        if not revealed:
            await q.answer("❌ Сначала откройте хотя бы одну клетку!", show_alert=True)
            return
        mult = _mines_multiplier(len(revealed))
        win = int(bet * mult)
        profit = win - bet
        db.update_balance(user_id, profit)
        db.record_game(user_id, True)
        await give_xp_notify(update, user_id, config.XP_PER_GAME + config.XP_PER_WIN)
        del user_sessions[user_id]
        field = _mines_render(revealed, mines, show_all=True)
        await q.edit_message_text(
...
async def _mines_new_game(q, user_id: int, bet: int):
    row = db.get_user(user_id)
    if row["balance"] < bet:
        await q.edit_message_text(f"❌ Недостаточно монет. У вас: {row['balance']}")
        return
    db.update_balance(user_id, -bet)
    import random as _rnd
    mines = _rnd.sample(range(25), MINES_COUNT)
    import uuid
    ...
    field = _mines_render([], mines)
    kb = _mines_kb([], mines, game_id)
    await q.edit_message_text(
        f"💣 <b>Мины 5×5</b>  |  Ставка: {bet}\n\n"
        f"{field}\n\n"
        f"✅ Открыто: 0 / 20\n"
        f"⚠️ Мин на поле: {MINES_COUNT}\n"
        f"Нажмите клетку чтобы открыть:",
        reply_markup=kb, parse_mode="HTML"
    )
                ...
                if to_row:
                    try:
                        await ctx.bot.send_message(
                            chat_id=int(to_id),
                            text=f"💰 <b>Получен трейд!</b>\n\n"
                                 f"Вам отправлено: <b>{amount} монет</b>\n"
                                 f"От игрока ID: <code>{user_id}</code>\n\n"
                                 f"🎮 Монеты зачислены в игровой баланс.",
                            parse_mode="HTML"
                        )
                    except Exception:
                        pass
                await update.message.reply_text(
                    f"✅ Трейд выполнен! Отправлено <b>{amount} монет</b>.",
                    parse_mode="HTML"
                )
            else:
                await update.message.reply_text(f"❌ {result}")
...
async def cmd_top(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    medals = ["🥇", "🥈", "🥉"] + ["🏅"] * 27
    lines = ["🏆 <b>ТОП-30 игроков</b>\n"]
    for i, row in enumerate(rows):
        medal = medals[i]
        raw = row["username"] or str(row["user_id"])
        lines.append(f"{medal} <b>{raw}</b> — {row['balance']} 💰  Ур.{row['level']}")
    await update.message.reply_text("\n".join(lines), parse_mode="HTML")
...
async def _handle_ocp_sessions(update: Update, session: dict, txt: str, uid: int) -> bool:
    ...
    if stype == "ocp_find":
        ...
        if txt.isdigit():
            ...
            conn = db.get_conn()
            cur = conn.cursor()
            cur.execute("SELECT user_id FROM users WHERE username=%s", (txt[1:],))
            row = cur.fetchone()
            cur.close(); conn.close()
            if row:
                target_id = row[0]
...
async def text_router(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if session:
        ...
        if stype == "slots_custom_bet":
            ...
            if row["balance"] < bet:
                return await update.message.reply_text(f"❌ Недостаточно монет. У вас: {row['balance']}")
            # Создаём фейковый callback_query-like объект через сообщение
            user_sessions[user.id] = {
                "type": "mines_pending_bet",
                "bet": bet,
            }
            # Запускаем игру через отдельное сообщение
            import random as _rnd, uuid
            mines = _rnd.sample(range(25), MINES_COUNT)
            game_id = uuid.uuid4().hex[:8]
            user_sessions[user.id] = {
                "type": "mines",
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(profile_callback,         pattern=r"^(profile_|deposit_)"))
