...
def get_conn():
    ...
    # Убираем sslmode из URL если есть, передаём параметром
    import re as _re
    ...
    try:
        ...
        if not cur.fetchone():
            ...
            if referrer_id:
                ...
                if cur.fetchone():
                    cur.execute("UPDATE users SET balance=balance+%s WHERE user_id=%s", (config.REFERRAL_BONUS, user_id))
                    cur.execute("UPDATE users SET balance=balance+%s WHERE user_id=%s", (config.REFERRAL_BONUS, referrer_id))
            conn.commit()
        else:
...
def get_all_user_ids() -> list:
    """Возвращает всех пользователей — включая ранее забаненных."""
    conn = get_conn()
    cur = conn.cursor()
    try:
...
def deposit_to_game(user_id: int, amount: int):
    ...
    try:
        ...
        cur.execute("INSERT INTO transfers (from_id,to_id,direction,amount,note,created_at) "
...
def get_top_users(limit: int = 30):
    ...
    cur.execute(
        """SELECT u.*, c.name AS clan_name
           ...
           ORDER BY u.game_balance DESC LIMIT %s""",
        (limit,)
    )
    rows = [_row(cur, r) for r in cur.fetchall()]
    cur.close(); conn.close()
