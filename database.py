...
def deposit_to_game(user_id: int, amount: int):
    ...
    cur.execute(
        "INSERT INTO transfers (user_id,from_id,to_id,direction,amount,note,created_at) "
        "VALUES (%s,%s,NULL,'deposit',%s,'Пополнение игры',%s)",
        (user_id, user_id, amount, int(time.time()))
    )
    conn.commit()
    cur.close(); conn.close()
...
def withdraw_from_game(user_id: int, amount: int):
    ...
    cur.execute(
        "INSERT INTO transfers (user_id,from_id,to_id,direction,amount,note,created_at) "
        "VALUES (%s,%s,NULL,'withdraw',%s,'Вывод из игры',%s)",
        (user_id, user_id, amount, int(time.time()))
    )
    conn.commit()
    cur.close(); conn.close()
...
def trade_coins(from_id: int, to_id: int, amount: int, fee: int = 0):
    ...
    cur.execute(
        "INSERT INTO transfers (user_id,from_id,to_id,direction,amount,note,created_at) "
        "VALUES (%s,%s,%s,'trade',%s,'Трейд между игроками',%s)",
        (from_id, from_id, to_id, amount, int(time.time()))
    )
    conn.commit()
    cur.close(); conn.close()
    return True, "ok"
...
def add_trade_history(from_id: int, to_id: int, amount: int, direction: str = "trade"):
    ...
    cur.execute(
        "INSERT INTO transfers (user_id,from_id,to_id,direction,amount,note,created_at) "
        "VALUES (%s,%s,%s,%s,%s,'Перевод через бот',%s)",
        (from_id, from_id, to_id, direction, amount, int(time.time()))
    )
    conn.commit()
    cur.close(); conn.close()
