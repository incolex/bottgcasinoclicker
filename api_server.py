...
class SaveStateRequest(BaseModel):
    ...
    delta: Optional[int] = None   # монеты накопленные с последнего сохранения
...
def save_game_state(req: SaveStateRequest):
    ...
    if client_delta > 0:
        # Клики/пассив — прибавляем дельту к текущему балансу в БД
        # Важно: берём max(db, client_base) чтобы не потерять пополнение через /ocp
        # client_coins уже включает последний известный клиенту баланс
        client_base = (client_coins - client_delta) if client_coins is not None else 0
        base = max(db_game_balance, client_base if client_base >= 0 else 0)
        new_balance = base + client_delta
    elif client_coins is not None and client_coins >= 0:
        # Точный баланс (после апгрейда/трейда/обмена)
        # КРИТИЧНО: если в БД баланс БОЛЬШЕ (пополнение через /ocp пока игрок был офлайн)
        # — не затираем его! Берём максимум.
        if client_coins == 0 and db_game_balance > 0:
            new_balance = db_game_balance  # не обнуляем!
        elif db_game_balance > client_coins:
            # В БД больше — значит был внешний депозит (/ocp), берём БД
