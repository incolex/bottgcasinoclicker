...
def save_game_state(req: SaveStateRequest):
    ...
    if client_delta > 0:
        ...
        # Защита: не позволяем уменьшить баланс если delta=0
        # (может быть гонка: клиент ещё не получил актуальный баланс)
        # Разрешаем уменьшение только если клиент прислал МЕНЬШЕ и delta=0
        # (апгрейд, трейд, обмен — они намеренно уменьшают)
        # Запрещаем если coins=0 при db>0 (баг инициализации)
