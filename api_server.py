...
def save_game_state(req: SaveStateRequest):
    ...
    # Если клиент прислал delta (накликанные монеты) — прибавляем к БД.
    # Это безопасно: нельзя "накликать" больше чем реально, т.к. апгрейды
    # сохраняются отдельно через coins (точный баланс).
    if req.delta is not None and int(req.delta) > 0:
        new_balance = db_game_balance + int(req.delta)
    elif req.coins is not None and int(req.coins) >= 0:
        # Апгрейд/трейд: точный баланс от клиента (delta=0 или отсутствует)
        new_balance = int(req.coins)
    else:
