...
def save_game_state(req: SaveStateRequest):
    ...
    if client_delta > 0:
        ...
        if db_game_balance == 0:
            # Новый игрок или первый успешный save: берём client_base как стартовую точку
            base = max(0, client_base)
        elif client_base > db_game_balance + 500:
            # Клиент сильно опередил DB (оффлайн-пополнение от /ocp): берём клиента
            base = client_base
        else:
...
def top_players(limit: int = 30):
    ...
    cur.execute(
        """SELECT u.user_id, u.username, u.game_balance, u.level,
           ...
           FROM users u
           WHERE u.is_banned=FALSE AND COALESCE(u.inactive_banned, FALSE)=FALSE
             AND u.game_balance > 0
...
def get_rank(user_id: int):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM users WHERE is_banned=FALSE AND COALESCE(inactive_banned,FALSE)=FALSE AND COALESCE(game_balance,0)>%s",
        (bal,)
    )
...
def top_extended(limit: int = 30):
    ...
    cur.execute(
        """SELECT u.user_id, u.username, u.game_balance, u.level,
           ...
           LEFT JOIN daily_data d ON u.user_id = d.user_id
           WHERE u.is_banned=FALSE
             AND COALESCE(u.inactive_banned, FALSE)=FALSE
             AND u.game_balance > 0
           ORDER BY u.game_balance DESC NULLS LAST
           LIMIT %s""",
        (min(limit, 100),)
