...
def init_db():
    ...
    for sql in [
        ...
        # Migrate existing diamonds from game_state JSON to column
        """UPDATE users SET diamonds = COALESCE((game_state::json->>'diamonds')::int, 0)
           WHERE diamonds = 0 AND game_state IS NOT NULL AND game_state != ''
           AND game_state::json->>'diamonds' IS NOT NULL""",
    ]:
        try:
...
def check_user(tg_id: int, username: str = ""):
    ...
    if not row:
        ...
        cur.execute(
            """INSERT INTO users
               (user_id,username,balance,game_balance,diamonds,game_state,xp,level,
                games_played,wins,losses,daily_last,created_at,updated_at,is_banned)
               VALUES (%s,%s,1000,0,0,NULL,0,1,0,0,0,0,%s,%s,FALSE)""",
...
def load_game_state(tg_id: int):
    ...
    db_diamonds = int(row.get("diamonds") or 0)
    ...
    if state:
        ...
        # Синхронизируем алмазы из колонки БД — берём максимум
        state_diamonds = int(state.get("diamonds") or 0)
        state["diamonds"] = max(db_diamonds, state_diamonds)
...
def save_game_state(req: SaveStateRequest):
    ...
    if not row:
        if req.tg_id and req.tg_id != 0:
            ...
            try:
                cur.execute(
                    """INSERT INTO users
                       (user_id,username,balance,game_balance,diamonds,xp,level,
                        games_played,wins,losses,daily_last,created_at,updated_at,is_banned)
                       VALUES (%s,%s,0,%s,0,0,1,0,0,0,0,%s,%s,FALSE) ON CONFLICT DO NOTHING""",
    ...
    cur.execute("SELECT diamonds FROM users WHERE user_id=%s", (req.tg_id,))
    diamonds_row = cur.fetchone()
    db_diamonds_col = int(diamonds_row["diamonds"] or 0) if diamonds_row else 0

    if not req.exact:
        # Берём максимум между колонкой БД, state в БД и клиентом
        db_diamonds_state = int(db_state.get("diamonds") or 0)
        final_diamonds = max(db_diamonds_col, db_diamonds_state, client_diamonds)
    else:
        # exact=True — клиент явно изменил (покупка/обмен) — доверяем клиенту, но не меньше 0
        final_diamonds = max(0, client_diamonds)

    state_to_save["diamonds"] = final_diamonds
    ...
    cur.execute(
        "UPDATE users SET game_balance=%s, diamonds=%s, game_state=%s, updated_at=%s WHERE user_id=%s",
        (new_balance, final_diamonds, json.dumps(state_to_save, ensure_ascii=False), int(time.time()), req.tg_id)
    )
    conn.commit(); cur.close(); conn.close()
    return {"ok": True, "db_game_balance": new_balance, "db_diamonds": final_diamonds}
...
def give_season_reward(req: SeasonRewardRequest):
    ...
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT game_state, diamonds FROM users WHERE user_id=%s", (req.user_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close()
    ...
    new_diamonds = int(row.get("diamonds") or 0) + req.diamonds
    state["diamonds"] = new_diamonds
    cur.execute("UPDATE users SET diamonds=%s, game_state=%s WHERE user_id=%s",
                (new_diamonds, json.dumps(state, ensure_ascii=False), req.user_id))
    conn.commit(); cur.close(); conn.close()
    return {"ok": True, "new_diamonds": new_diamonds}
...
def claim_achievement(req: AchClaimRequest):
    ...
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT game_state, diamonds FROM users WHERE user_id=%s", (req.tg_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close()
        raise HTTPException(404, "Пользователь не найден")
    ...
    cur.execute("SELECT game_balance, diamonds FROM users WHERE user_id=%s", (req.tg_id,))
    nb = cur.fetchone()
    conn.commit(); cur.close(); conn.close()
    ...
    return {
        ...
        "db_diamonds": int(nb["diamonds"]) if nb else 0,
        "ach_id": req.ach_id
    }


# ── Direct diamonds endpoint ─────────────────────────────────

@app.get("/api/diamonds/{tg_id}")
def get_diamonds(tg_id: int):
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT diamonds FROM users WHERE user_id=%s", (tg_id,))
    row = cur.fetchone()
    cur.close(); conn.close()
    if not row:
        raise HTTPException(404, "Не найден")
    return {"ok": True, "diamonds": int(row["diamonds"] or 0)}


class DiamondUpdateRequest(BaseModel):
    tg_id: int
    delta: int  # положительный = добавить, отрицательный = списать
    reason: str = ""

@app.post("/api/diamonds/update")
def update_diamonds(req: DiamondUpdateRequest):
    """Прямое изменение алмазов с синхронизацией в game_state."""
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT diamonds, game_state FROM users WHERE user_id=%s FOR UPDATE", (req.tg_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close()
        raise HTTPException(404, "Не найден")
    new_diamonds = max(0, int(row["diamonds"] or 0) + req.delta)
    # Синхронизируем в game_state
    state = {}
    if row["game_state"]:
        try: state = json.loads(row["game_state"])
        except: pass
    state["diamonds"] = new_diamonds
    cur.execute(
        "UPDATE users SET diamonds=%s, game_state=%s, updated_at=%s WHERE user_id=%s",
        (new_diamonds, json.dumps(state, ensure_ascii=False), int(time.time()), req.tg_id)
    )
    conn.commit(); cur.close(); conn.close()
    logger.info(f"[DIAMONDS] uid={req.tg_id} delta={req.delta} new={new_diamonds} reason={req.reason}")
