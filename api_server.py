...
def save_game_state(req: SaveStateRequest):
    ...
    # Обновляем game_balance если клиент прислал больше чем в БД (от кликов)
    if new_coins is not None and int(new_coins) > db_game_balance:
...
def deposit_to_game(req: DepositRequest):
    ...
    cur.execute(
        "INSERT INTO transfers (user_id,from_id,direction,amount,note,created_at) VALUES (%s,%s,'deposit',%s,'Пополнение игры',%s)",
...
def withdraw_from_game(req: DepositRequest):
    ...
    cur.execute(
        "INSERT INTO transfers (user_id,from_id,direction,amount,note,created_at) VALUES (%s,%s,'withdraw',%s,'Вывод из игры',%s)",
        (req.tg_id, req.tg_id, req.amount, int(time.time()))
    )
    conn.commit(); cur.close(); conn.close()
    return {"ok": True, "new_game_balance": new_game, "new_bot_balance": new_bot}
...
async def casino_deposit(req: CasinoDepositRequest):
    ...
    cur.execute(
        "INSERT INTO transfers (user_id,from_id,direction,amount,note,created_at) VALUES (%s,%s,'casino',%s,'Казино через игру',%s)",
        (req.tg_id, req.tg_id, req.amount, int(time.time()))
    )
    conn.commit(); cur.close(); conn.close()
...
async def trade_coins(req: TradeRequest):
    ...
    cur.execute(
        "INSERT INTO transfers (user_id,from_id,to_id,direction,amount,note,created_at) VALUES (%s,%s,%s,'trade',%s,'Трейд',%s)",
        (req.from_id, req.from_id, to_id, req.amount, int(time.time()))
    )
    conn.commit()
