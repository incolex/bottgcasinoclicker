...
def get_conn():
    ...
    # Render даёт postgresql://, psycopg2 иногда требует postgres://
    # Также добавляем sslmode если не указан
...
class SaveStateRequest(BaseModel):
    tg_id: int
    state: dict
    coins: Optional[int] = None  # Игнорируем — game_balance только через транзакции
...
def save_game_state(req: SaveStateRequest):
    ...
    # Сохраняем только game_state (UI-состояние), НЕ трогаем game_balance
    # game_balance меняется только через /api/trade, /api/casino_deposit, /api/deposit
    state_to_save = dict(req.state)
    state_to_save.pop("coins", None)  # убрать coins из state — берётся из БД
    ...
    conn.commit(); cur.close(); conn.close()
    # Возвращаем актуальный баланс из БД
...
def top_players(limit: int = 30):
    conn = get_conn(); cur = conn.cursor()
    cur.execute(
        """SELECT user_id, username, game_balance, level
           FROM users WHERE is_banned=FALSE
           ORDER BY game_balance DESC NULLS LAST
           LIMIT %s""",
        (min(limit, 100),)
    )
    rows = cur.fetchall()
...
def deposit_to_game(req: DepositRequest):
    ...
    cur.execute(
        "INSERT INTO transfers (from_id,direction,amount,note,created_at) VALUES (%s,'deposit',%s,'Пополнение игры',%s)",
        (req.tg_id, req.amount, int(time.time()))
    )
    conn.commit(); cur.close(); conn.close()
...
def withdraw_from_game(req: DepositRequest):
    ...
    cur.execute(
        "INSERT INTO transfers (from_id,direction,amount,note,created_at) VALUES (%s,'withdraw',%s,'Вывод из игры',%s)",
        (req.tg_id, req.amount, int(time.time()))
    )
    conn.commit(); cur.close(); conn.close()
    return {"ok": True, "new_game_balance": new_game, "new_bot_balance": new_bot}
...
async def casino_deposit(req: CasinoDepositRequest):
    """Списать монеты из game_balance для казино-бота."""
    if req.amount <= 0:
        raise HTTPException(400, "Сумма должна быть > 0")
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT game_balance, username FROM users WHERE user_id=%s FOR UPDATE", (req.tg_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close()
        raise HTTPException(404, "Пользователь не найден")
    ...
    cur.execute(
        ...
        (new_game, int(time.time()), req.tg_id)
    )
    cur.execute(
        "INSERT INTO transfers (from_id,direction,amount,note,created_at) VALUES (%s,'casino',%s,'Казино через игру',%s)",
        (req.tg_id, req.amount, int(time.time()))
    )
    conn.commit(); cur.close(); conn.close()
    ...
    asyncio.ensure_future(notify_user(
        req.tg_id,
        f"🎰 <b>Казино-бот!</b>\n\n"
        f"Списано: <b>{req.amount} монет</b> из игры\n"
        f"🎮 Остаток в игре: <b>{new_game} монет</b>"
    ))
    return {"ok": True, "new_balance": new_game}
