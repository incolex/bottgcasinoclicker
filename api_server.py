...
def top_players(limit: int = 30):
    ...
    return {
        "top": [
            {
                "user_id":  r[0],
                "username": r[1] or f"id{r[0]}",
                "coins":    (r[2] or 0) + (r[3] or 0),
                "level":    r[4] or 1,
            }
            for r in rows
        ]
...
def get_rank(user_id: int):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM users WHERE is_banned=FALSE AND (balance+COALESCE(game_balance,0))>%s",
        (total,)
    )
    cnt = cur.fetchone()[0]
    cur.close(); conn.close()
...
def global_stats():
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT COUNT(*), SUM(balance) FROM users WHERE is_banned=FALSE")
    row = cur.fetchone()
    cur.close(); conn.close()
    return {"total_players": row[0] or 0, "total_coins": int(row[1] or 0)}


# ── Авторегистрация / проверка ────────────────────────────────
    ...
    if not row:
        ...
        cur.execute(
            """INSERT INTO users
               ...
               VALUES (%s,%s,1000,0,NULL,0,1,0,0,0,0,NULL,%s,%s,FALSE)""",
...
def save_game_state(req: SaveStateRequest):
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT user_id, is_banned FROM users WHERE user_id=%s", (req.tg_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close()
        raise HTTPException(status_code=404, detail="Пользователь не найден.")
