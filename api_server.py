...
def init_db():
    ...
    for sql in [
        ...
        try:
            ...
            conn.rollback()
    # Settings table for season timer etc
    cur.execute("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)")
    # Performance indexes for top queries
    for idx_sql in [
...
def load_game_state(tg_id: int):
    ...
    if click_ban_until > 0 and click_ban_until < int(time.time()):
        ...
        click_ban_reason = ""
        try:
            cur.execute("UPDATE users SET click_ban_until=0, click_ban_reason='' WHERE user_id=%s", (tg_id,))
            conn.commit()
...
class SaveStateRequest(BaseModel):
    ...
    exact: Optional[bool] = False  # True = клиент намеренно потратил (апгрейд/трейд), принять как точный баланс

# Античит: максимальный delta за один save-запрос
# clickPower макс ~361, combo x1.5, ~15 кликов/сек, 5 сек → ~40k. С запасом x5 = 200k
_MAX_DELTA_PER_SAVE = 200_000
_MAX_BALANCE = 10_000_000_000  # 10 млрд — физический максимум
...
def save_game_state(req: SaveStateRequest):
    ...
    row = cur.fetchone()
    if not row:
        # Auto-register if /check failed due to cold start / network error
    ...
    # ── АНТИЧИТ: проверяем и зажимаем входящие данные ──────────
    client_coins = int(req.coins) if req.coins is not None else None
    client_delta = int(req.delta) if req.delta is not None else 0

    # 1. Delta не может быть отрицательной
    if client_delta < 0:
        client_delta = 0

    # 2. Delta не может превышать физический максимум за один интервал
    ...
    # 3. Итоговый баланс не может превышать глобальный максимум
    # (exact=True при апгрейде — не блокируем уменьшение, но проверяем что не увеличение)
    if req.exact and client_coins is not None:
        if client_coins > db_game_balance:
            # Клиент пытается exact-сохранить БОЛЬШЕ чем в БД — читерство
    ...
    # ── Считаем новый баланс — БД является источником истины ───
    if client_delta > 0:
        # /ocp пополняет только bot balance, не game_balance.
        # Клиент никогда не может законно "опередить" БД — всегда берём БД.
        base = db_game_balance
    ...
    # Никогда не уходим в минус, и не превышаем физический максимум
...
def get_rank(user_id: int):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM users WHERE (is_banned IS NULL OR is_banned=FALSE) AND (inactive_banned IS NULL OR inactive_banned=FALSE) AND COALESCE(game_balance,0)>%s",
        (bal,)
    )
...
def deposit_to_game(req: DepositRequest):
    """ОТКЛЮЧЕНО: пополнение game_balance возможно только через /ocp в боте."""
    raise HTTPException(403, "Пополнение игры доступно только через бота командой /ocp")


# ── Пополнение balance (бот-баланса) через OCP — только бот ─────────────────
# Бот вызывает этот эндпоинт с секретным токеном BOT_TOKEN в заголовке X-Bot-Token

class BotTopupRequest(BaseModel):
    tg_id: int
    amount: int
    note: str = "Пополнение через /ocp"

@app.post("/api/bot/topup")
async def bot_topup(req: BotTopupRequest, x_bot_token: Optional[str] = None):
    """Пополнение bot-balance (поле balance) — вызывается только ботом через /ocp.
    Заголовок X-Bot-Token должен совпадать с BOT_TOKEN из env."""
    from fastapi import Header
    # Проверка токена (передаётся в заголовке X-Bot-Token)
    if not BOT_TOKEN or x_bot_token != BOT_TOKEN:
        raise HTTPException(403, "Forbidden")
    if req.amount <= 0:
        raise HTTPException(400, "Сумма должна быть > 0")
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT user_id, balance, is_banned FROM users WHERE user_id=%s FOR UPDATE", (req.tg_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close()
        raise HTTPException(404, "Пользователь не найден")
    ...
    cur.execute(
        ...
        (new_bot, int(time.time()), req.tg_id)
    )
    cur.execute(
        "INSERT INTO transfers (from_id,to_id,direction,amount,note,created_at) VALUES (0,%s,'bot_topup',%s,%s,%s)",
        (req.tg_id, req.amount, req.note[:500], int(time.time()))
    )
    conn.commit(); cur.close(); conn.close()
    logger.info(f"[BOT-TOPUP] tg_id={req.tg_id} amount={req.amount} note={req.note}")
    return {"ok": True, "new_bot_balance": new_bot}
...
async def trade_coins(req: TradeRequest):
    ...
    # Блокируем строки в фиксированном порядке (по user_id) для избежания deadlock
...
# ══════════════════════════════════════════════════════════════
# МИНЫ — серверная логика
# ══════════════════════════════════════════════════════════════

import random as _random

# Множители выплат в зависимости от числа мин и открытых ячеек
def _mines_multiplier(mines: int, opened: int) -> float:
    """Честный расчёт множителя: edge ~2% у казино."""
    total = 25
    safe = total - mines
    if opened <= 0 or opened > safe:
        return 1.0
    mult = 1.0
...
def mines_start(req: MinesStartRequest):
    """Начать игру в мины. Списываем ставку, генерируем поле."""
    if not (1 <= req.mines <= 24):
        raise HTTPException(400, "mines должно быть от 1 до 24")
    if req.bet <= 0:
        raise HTTPException(400, "Ставка должна быть > 0")

    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT game_balance, is_banned FROM users WHERE user_id=%s FOR UPDATE", (req.tg_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close()
        raise HTTPException(404, "Пользователь не найден")
    if row["is_banned"]:
        cur.close(); conn.close()
        raise HTTPException(403, "Аккаунт заблокирован")
    if int(row["game_balance"] or 0) < req.bet:
        cur.close(); conn.close()
        raise HTTPException(400, f"Недостаточно монет. Баланс: {row['game_balance']}")

    # Убить старую сессию если есть
    cur.execute("DELETE FROM mines_sessions WHERE user_id=%s", (req.tg_id,))

    # Списать ставку
    new_bal = int(row["game_balance"]) - req.bet
    cur.execute("UPDATE users SET game_balance=%s, updated_at=%s WHERE user_id=%s",
                (new_bal, int(time.time()), req.tg_id))

    # Генерировать позиции мин (25 ячеек, 0..24)
    positions = _random.sample(range(25), req.mines)
    cur.execute(
        "INSERT INTO mines_sessions (user_id,bet,mines,mine_positions,opened_cells,created_at) VALUES (%s,%s,%s,%s,'',%s)",
        (req.tg_id, req.bet, req.mines, json.dumps(sorted(positions)), int(time.time()))
    )
    conn.commit(); cur.close(); conn.close()
    ...
    return {
        "ok": True,
        "new_balance": new_bal,
        "multiplier": _mines_multiplier(req.mines, 0),
    }
...
def mines_open(req: MinesOpenRequest):
    """Открыть ячейку. Если мина — игра проиграна."""
    if not (0 <= req.cell <= 24):
        raise HTTPException(400, "cell должно быть от 0 до 24")
    ...
    if not session:
        cur.close(); conn.close()
        raise HTTPException(404, "Нет активной игры. Начните новую.")

    mine_positions = json.loads(session["mine_positions"])
    opened_str = session["opened_cells"] or ""
    opened = [int(x) for x in opened_str.split(",") if x.strip().isdigit()]

    if req.cell in opened:
        cur.close(); conn.close()
        raise HTTPException(400, "Ячейка уже открыта")

    if req.cell in mine_positions:
        # Взрыв — удаляем сессию, баланс уже списан
        cur.execute("DELETE FROM mines_sessions WHERE user_id=%s", (req.tg_id,))
        conn.commit(); cur.close(); conn.close()
        return {
            "ok": True,
    ...
    # Безопасная ячейка
    opened.append(req.cell)
    safe = 25 - session["mines"]
    new_opened_str = ",".join(str(x) for x in opened)
    mult = _mines_multiplier(session["mines"], len(opened))
    potential_win = int(session["bet"] * mult)

    if len(opened) >= safe:
        # Все безопасные открыты — авто-кешаут
        cur.execute("DELETE FROM mines_sessions WHERE user_id=%s", (req.tg_id,))
        cur.execute("UPDATE users SET game_balance=game_balance+%s, updated_at=%s WHERE user_id=%s",
        ...
        return {
            ...
            "new_balance": int(nb["game_balance"]) if nb else 0,
            "mine_positions": mine_positions,
        }

    # Продолжаем игру
    cur.execute("UPDATE mines_sessions SET opened_cells=%s WHERE user_id=%s",
                (new_opened_str, req.tg_id))
    conn.commit()
    cur.execute("SELECT game_balance FROM users WHERE user_id=%s", (req.tg_id,))
    nb = cur.fetchone()
    cur.close(); conn.close()
    return {
        ...
        "current_balance": int(nb["game_balance"]) if nb else 0,
    }
...
def mines_cashout(req: MinesCashoutRequest):
    ...
    if not session:
        cur.close(); conn.close()
        raise HTTPException(404, "Нет активной игры")

    opened_str = session["opened_cells"] or ""
    opened = [int(x) for x in opened_str.split(",") if x.strip().isdigit()]
    if not opened:
        # Ни одной открытой — возвращаем ставку
        win_amount = session["bet"]
        mult = 1.0
    else:
        mult = _mines_multiplier(session["mines"], len(opened))
        win_amount = int(session["bet"] * mult)

    mine_positions = json.loads(session["mine_positions"])
    cur.execute("DELETE FROM mines_sessions WHERE user_id=%s", (req.tg_id,))
    cur.execute("UPDATE users SET game_balance=game_balance+%s, updated_at=%s WHERE user_id=%s",
                (win_amount, int(time.time()), req.tg_id))
    cur.execute("SELECT game_balance FROM users WHERE user_id=%s", (req.tg_id,))
    nb = cur.fetchone()
    conn.commit(); cur.close(); conn.close()
    return {
...
@app.get("/api/mines/state/{tg_id}")
def mines_get_state(tg_id: int):
    """Получить текущее состояние игры в мины (восстановление после перезагрузки)."""
    conn = get_conn(); cur = conn.cursor()
...
async def claim_streak(req: StreakClaimRequest):
    ...
    today = datetime.date.today().isoformat()  # "2025-01-15"
