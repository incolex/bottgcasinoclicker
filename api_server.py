...
def register_user(req: RegisterRequest):
    ...
    uname = req.username or f"id{req.tg_id}"
    cur.execute(
        """INSERT INTO users (user_id,username,balance,game_balance,game_state,xp,level,
           ...
           VALUES (%s,%s,0,0,NULL,0,1,0,0,0,0,NULL,%s,%s)""",
...
def check_user(tg_id: int, username: str = ""):
    ...
    row = cur.fetchone()
    if not row:
        # Авторегистрация — создаём пользователя с 1 монетой на балансе
        uname = username or f"id{tg_id}"
        cur.execute(
            """INSERT INTO users (user_id,username,balance,game_balance,game_state,xp,level,
               games_played,wins,losses,daily_last,referrer_id,created_at,updated_at)
               VALUES (%s,%s,1,0,NULL,0,1,0,0,0,0,NULL,%s,%s)""",
            (tg_id, uname, int(time.time()), int(time.time()))
        )
        conn.commit()
        cur.close(); conn.close()
        return {"registered": True, "username": uname, "auto_registered": True}
    cur.close(); conn.close()
