...
def load_game_state(tg_id: int):
    ...
    # Извлекаем claimed_achs из state для возврата клиенту
    claimed_achs = {}
    if state and state.get("claimedAchs"):
...
def save_game_state(req: SaveStateRequest):
    ...
    if client_delta > 0:
        ...
        if db_game_balance == 0:
            base = max(0, client_base)
        elif client_base > db_game_balance + 500:
            base = client_base
        else:
    ...
    # ── ФИКС АЛМАЗОВ: читаем текущий game_state из БД и берём MAX diamonds ──
    # Это предотвращает потерю алмазов при конкурентных сохранениях
    client_diamonds = int(state_to_save.get("diamonds") or 0)
    if client_diamonds > 0:
        ...
        if gs_row and gs_row.get("game_state"):
            try:
                ...
                # Берём максимум — алмазы не могут уменьшиться сами по себе
                # (уменьшение происходит только через явные покупки с exact=True)
                if db_diamonds > client_diamonds and not req.exact:
                    state_to_save["diamonds"] = db_diamonds
                    logger.info(f"[DIAMONDS] tg_id={req.tg_id} kept db_diamonds={db_diamonds} over client={client_diamonds}")
            except Exception:
                pass

    # ── ФИКС АЧИВОК: мержим claimedAchs из БД с клиентскими (не затираем) ──
    client_achs = state_to_save.get("claimedAchs") or {}
    if client_achs:
        cur.execute("SELECT game_state FROM users WHERE user_id=%s", (req.tg_id,))
        gs_row2 = cur.fetchone()
        if gs_row2 and gs_row2.get("game_state"):
            try:
                db_state2 = json.loads(gs_row2["game_state"])
                db_achs = db_state2.get("claimedAchs") or {}
                # Объединяем: если в БД есть — не затираем
                merged = {**client_achs, **db_achs}
                state_to_save["claimedAchs"] = merged
            except Exception:
                pass
...
def claim_quest(req: QuestClaimRequest):
    ...
    # Максимальный разрешённый reward за квест — защита от накрутки
    MAX_QUEST_REWARD = 500
    clamped_reward = min(req.reward, MAX_QUEST_REWARD)
    ...
    if row:
        ...
        # Если дата другая — квесты сбрасываются (новый день)
    ...
    cur.execute("UPDATE users SET game_balance=game_balance+%s WHERE user_id=%s", (clamped_reward, req.tg_id))
    cur.execute("SELECT game_balance FROM users WHERE user_id=%s", (req.tg_id,))
    nb = cur.fetchone()
    conn.commit(); cur.close(); conn.close()
...
# Список допустимых ачивок и их максимальных наград
_VALID_ACHS = {
    "click100":  5000,
    "click1k":   15000,
    "click10k":  50000,
...
def claim_achievement(req: AchClaimRequest):
    ...
    # Валидация: только известные ачивки с правильным reward
    max_reward = _VALID_ACHS.get(req.ach_id)
    if max_reward is None:
        raise HTTPException(400, f"Unknown achievement: {req.ach_id}")
    clamped_reward = min(req.reward, max_reward)

    conn = get_conn(); cur = conn.cursor()

    # Читаем текущий game_state
    cur.execute("SELECT game_state FROM users WHERE user_id=%s", (req.tg_id,))
    row = cur.fetchone()
    if not row:
        cur.close(); conn.close()
    ...
    # Проверяем что не получено ранее
    ...
    cur.execute(
        ...
        (clamped_reward, json.dumps(state, ensure_ascii=False), int(time.time()), req.tg_id)
    )
    cur.execute("SELECT game_balance FROM users WHERE user_id=%s", (req.tg_id,))
    nb = cur.fetchone()
    conn.commit(); cur.close(); conn.close()
    logger.info(f"[ACH_CLAIM] tg_id={req.tg_id} ach={req.ach_id} reward={clamped_reward}")
